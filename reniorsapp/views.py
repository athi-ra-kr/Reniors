...
from .models import (
    ...
    QuickServiceEnquiry, FacilityEnquiry, PackageEnquiry, FacilityImage, FacilityVideo,Customer
)
...
def list_country(request):
    country_list = Country.objects.all().order_by('-id')
    paginator = Paginator(country_list, 7)  # Show 7 countries per page
...
def list_state(request):
    ...
    paginator = Paginator(state_list, 7)  # Show 7 states per page
    page_number = request.GET.get('page')
...
def list_district(request):
    district_list = District.objects.all().order_by('-id')
    paginator = Paginator(district_list, 7)  # Show 7 districts per page
    page_number = request.GET.get('page')
...
def list_city(request):
    ...
    paginator = Paginator(city_list, 7)  # Show 7 cities per page
    page_number = request.GET.get('page')
...
def list_quick_services(request):
    services_list = QuickService.objects.all().order_by('-id')
...
def package_list(request):
    packages_list = Package.objects.all().order_by('-id')
...
@admin_required
def customer_package_list(request):
    packages = Customer.objects.all()
    return render(request, 'customer_package_list.html', {'packages': packages})

@admin_required
def customer_package_add(request):
    if request.method == 'POST':
        Customer.objects.create(
            ...
            package_features=request.POST['package_features'],
            support_manager_name=request.POST['support_manager_name'],
            support_manager_contact=request.POST['support_manager_contact'],
        )
        return redirect('customer_package_list')
    return render(request, 'customer_package_form.html', {'action': 'Add'})


@admin_required
def customer_package_edit(request, pk):
    package = get_object_or_404(Customer, pk=pk)
    if request.method == 'POST':
        package.name = request.POST['name']
        package.mpin = request.POST['mpin']
        package.package_name = request.POST['package_name']
        package.validity_date = request.POST['validity_date']
        package.package_features = request.POST['package_features']
        package.support_manager_name = request.POST['support_manager_name']
        package.support_manager_contact = request.POST['support_manager_contact']
        package.save()
        return redirect('customer_package_list')
    return render(request, 'customer_package_form.html', {'action': 'Edit', 'package': package})



@admin_required
def customer_package_delete(request, pk):
    package = get_object_or_404(Customer, pk=pk)
    package.delete()
    return redirect('customer_package_list')
...
def customer_portal(request):
    ...
    if request.method == 'POST':
        ...
        try:
            customer = Customer.objects.get(mpin=mpin)
        except Customer.DoesNotExist:
            error = "No customer found with that MPIN."
...
class CustomerByMpinView(APIView):
    ...
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter(
                'mpin',
                openapi.IN_QUERY,
                description="MPIN provided by admin",
    ...
    def get(self, request):
        mpin = request.query_params.get('mpin')
        if not mpin:
            return Response({'error': 'MPIN is required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'error': 'Invalid MPIN'}, status=status.HTTP_404_NOT_FOUND)
